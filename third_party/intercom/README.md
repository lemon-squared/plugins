...
Runtime plugin that connects agents to [Intercom](https://www.intercom.com) through Intercom's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.0 against Intercom with Dynamic Client Registration (DCR) and PKCE. Runtime registers itself and prompts for Intercom sign-in when the plugin connects — there is no API key or client ID to configure.
