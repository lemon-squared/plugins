...
Runtime plugin that connects agents to [Juicebox](https://juicebox.ai) through Juicebox's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.0 against Juicebox with Dynamic Client Registration (DCR). Runtime registers itself and prompts for Juicebox sign-in when the plugin connects — there is no API key or client ID to configure.
