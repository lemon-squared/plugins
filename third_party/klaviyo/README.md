...
Runtime plugin that connects agents to [Klaviyo](https://www.klaviyo.com) through Klaviyo's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth with Dynamic Client Registration. Runtime registers itself and prompts for Klaviyo sign-in when the plugin connects — there is no API key or client ID to configure.
