...
Runtime plugin that connects agents to [Clay](https://www.clay.com) through Clay's official hosted [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 against Clay with Dynamic Client Registration (DCR) and PKCE. Runtime registers itself and prompts for Clay sign-in when the plugin connects — there is no API key or client ID to configure.
...
- Connections appear in your Clay workspace's MCP client list, labeled with the Runtime client name.
