...
Runtime plugin that connects agents to [Calendly](https://calendly.com) through Calendly's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 with PKCE and Dynamic Client Registration. Runtime registers itself and prompts for Calendly sign-in when the plugin connects — there is no client ID or personal access token to configure. Calendly does not accept personal access tokens on the MCP server.
