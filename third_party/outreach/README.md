...
Runtime plugin that connects agents to [Outreach](https://www.outreach.ai) through Outreach's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 against Outreach with Dynamic Client Registration (DCR) and PKCE. Runtime registers itself and prompts for Outreach sign-in when the plugin connects — there is no API key or client ID to configure.
