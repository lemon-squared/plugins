...
Runtime plugin that connects agents to [Otter.ai](https://otter.ai) through Otter.ai's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth. Runtime prompts for Otter sign-in when the plugin connects. Otter does not offer a public API key, so OAuth is the only path.
