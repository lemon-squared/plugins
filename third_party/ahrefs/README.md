...
Runtime plugin that connects agents to [Ahrefs](https://ahrefs.com) through Ahrefs's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth. Runtime prompts for Ahrefs sign-in when the plugin connects, and the consent screen mints an MCP-scoped key on the Ahrefs side. Leave any OAuth client ID and secret blank.
