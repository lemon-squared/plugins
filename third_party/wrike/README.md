...
Runtime plugin that connects agents to [Wrike](https://www.wrike.com) through Wrike's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Wrike's MCP server does not offer Dynamic Client Registration, so a token is the supported path for clients like Runtime rather than a browser OAuth flow.
