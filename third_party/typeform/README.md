...
Runtime plugin that connects agents to [Typeform](https://www.typeform.com) through Typeform's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth. Runtime prompts for Typeform sign-in when the plugin connects. Personal access tokens are explicitly rejected by the MCP server, so OAuth is the only path.
