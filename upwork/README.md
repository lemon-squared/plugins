...
Runtime plugin that connects agents to [Upwork](https://www.upwork.com) through Upwork's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 with Dynamic Client Registration. Runtime registers itself and prompts for Upwork sign-in when the plugin connects — there is no API key or client ID to configure.
