...
Runtime plugin that connects agents to [Smartsheet](https://www.smartsheet.com) through Smartsheet's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
- Smartsheet's own Runtime instructions route through `npx mcp-remote`. That is unnecessary here — Runtime speaks streamable HTTP natively and sends the `Authorization` header directly.
