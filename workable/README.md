...
Runtime plugin that connects agents to [Workable](https://www.workable.com) through Workable's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.0 with dynamic discovery and Dynamic Client Registration. Runtime registers itself and prompts for Workable sign-in when the plugin connects — there is no API key to configure.
...
Workable's MCP OAuth checks the client's redirect URI against a pre-approved list. Workable names Runtime as a supported client, but if sign-in fails with a **redirect URI mismatch**, ask Workable Support to allow Runtime's two fixed callbacks:
