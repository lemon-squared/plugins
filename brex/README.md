...
Runtime plugin that connects agents to [Brex](https://www.brex.com) through Brex's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth with Dynamic Client Registration. Runtime registers itself and prompts for Brex sign-in when the plugin connects — there is no API key or client ID to configure.
