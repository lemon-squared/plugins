...
Runtime plugin that connects agents to [Zoom](https://zoom.us) through Zoom's official hosted [Model Context Protocol](https://modelcontextprotocol.io/) servers.
...
Zoom's MCP servers only support manual client registration — Dynamic Client Registration and Client ID Metadata Documents are not accepted — so an administrator has to register Runtime as a Zoom app before anyone can connect.
...
5. Complete the Zoom OAuth login when Runtime prompts.
