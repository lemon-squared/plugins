...
Runtime plugin that connects agents to [Similarweb](https://www.similarweb.com) through Similarweb's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
- Similarweb's own Runtime guide routes through `npx mcp-remote`. That is not necessary — Runtime connects to the remote endpoint directly and sends the `api-key` header, which is the same shape Similarweb documents for Microsoft Copilot Studio.
...
- Runtime integration guide: https://developers.similarweb.com/docs/cursor-mcp-integration
