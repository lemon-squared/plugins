...
Runtime plugin that connects agents to [Semrush](https://www.semrush.com) through Semrush's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 and the client registers itself, so Runtime just prompts for Semrush sign-in when the plugin connects — there is no client ID to configure.
