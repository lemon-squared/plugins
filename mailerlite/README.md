...
Runtime plugin that connects agents to [MailerLite](https://www.mailerlite.com) through MailerLite's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth. Runtime prompts for MailerLite sign-in when the plugin connects — there is no API key or token to paste.
...
- The server exposes close to 40 tools. MailerLite notes that Runtime's free plan caps a single connector at 40 active tools, so you may see an "Exceeding total tools limit" warning there.
