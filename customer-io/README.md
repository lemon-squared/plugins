...
Runtime plugin that connects agents to [Customer.io](https://customer.io) through Customer.io's official remote [Model Context Protocol](https://modelcontextprotocol.io/) server.
...
Auth is OAuth 2.1 with PKCE and Dynamic Client Registration. Runtime registers itself and prompts for Customer.io sign-in when the plugin connects — there is no API key or client ID to configure. During the flow you choose which workspaces and scopes to grant.
