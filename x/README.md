...
Runtime plugin that connects agents to the [X API](https://docs.x.com) through X's official hosted [Model Context Protocol](https://modelcontextprotocol.io/) server at `https://api.x.com/mcp`.
...
No token to paste — the plugin ships with X's OAuth client ID and requests the scopes above. On first use, Runtime opens a browser window where you sign in to X and approve access. The `offline.access` scope lets Runtime refresh the session automatically, so you only sign in once.
