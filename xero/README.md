...
Runtime plugin that connects agents to [Xero](https://www.xero.com) through Xero's official [Model Context Protocol](https://modelcontextprotocol.io/) server, run locally by Runtime.
...
- This is a local stdio server, so `npx` has to be available on the machine running Runtime. It downloads `@xeroapi/xero-mcp-server` on first run.
- Xero's own FAQ says the server works with any client supporting local stdio servers, and that its testing was done with Claude Desktop and Runtime.
